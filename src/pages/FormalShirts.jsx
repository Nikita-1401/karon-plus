...
const FormalShirts = () => {
  const [adminFormalProducts, setAdminFormalProducts] = useState([]);

  /* ================= ADMIN FORMAL PRODUCTS ================= */

  const loadAdminFormalProducts = () => {
    try {
      const savedProducts =
        JSON.parse(
      ...
      const onlyFormalProducts = Array.isArray(savedProducts)
        ? savedProducts
            .filter(
            ...
            .map((product) => ({
              ...product,
              subtitle:
                product.subtitle || "Formal Collection",
            }))
        : [];

      setAdminFormalProducts(onlyFormalProducts);
    } catch (error) {
      console.error(
        "Unable to load formal products:",
        error
      );

      setAdminFormalProducts([]);
    }
  };

  useEffect(() => {
    loadAdminFormalProducts();
          ...
          {allFormalProducts.map((product) => (
            ...
            >
              ...
              <Link
                ...
                className="relative block overflow-hidden bg-[#d8cfc3] h-[315px] sm:h-[270px] lg:h-[270px] xl:h-[280px]"
              >
                <img
                  src={product.image}
                  alt={product.name}
